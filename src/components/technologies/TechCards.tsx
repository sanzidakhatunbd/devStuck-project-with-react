...
const TechCards = ({ tech, selectedTeches, setSelectedTeches }: TechCardsProps) => {
    const isSelected = selectedTeches.some((t) => t.id === tech.id);

    const handleAddToStack = () => {
        setSelectedTeches((prev) => [...prev, tech]);
    ...
    return (
        <div
            className={`flex flex-col rounded-2xl border bg-white p-4 ${isSelected ? "border-pink-400" : "border-gray-200"
                }`}
        >
            ...
            <div className="mt-4 flex flex-1 flex-col">
                ...
                <button onClick={handleAddToStack} className={`mt-4 w-full rounded-lg py-2.5 text-sm font-medium transition ${isSelected ? "bg-pink-100 text-pink-600" : "bg-gray-900 text-white hover:bg-gray-950"}`} disabled={isSelected}>
